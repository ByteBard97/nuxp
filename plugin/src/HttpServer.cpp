...
void HttpServer::ConfigureRoutes() {
    ...
    svr.Post("/api/call", [](const httplib::Request& req, httplib::Response& res) {
        try {
            ...
            json result = MainThreadDispatch::Run([&]() -> json {
                // Route to the appropriate suite wrapper
                // This will be expanded as more suite wrappers are generated
                json routeResult = {
                    {"success", true},
                    {"suite", suite},
                    {"method", method},
                    {"message", "Method routing not yet implemented for this suite"}
                };
                return routeResult;
            });
    ...
    svr.Post(R"(/(\w+)/(\w+))", [](const httplib::Request& req, httplib::Response& res) {
        try {
            ...
            json result = MainThreadDispatch::Run([&]() -> json {
                // Route to the appropriate generated wrapper
                // The generated wrappers will be called here based on suite/method
                json routeResult = {
                    {"success", true},
                    {"suite", suite},
                    {"method", method},
                    {"args", args},
                    {"message", "Endpoint not yet wired to generated code"}
                };
                return routeResult;
            });
    ...
    svr.Post("/handles/invalidate", [](const httplib::Request&, httplib::Response& res) {
        ...
        json response = {
            {"success", true},
            {"message", "All handles invalidated"}
        };
        res.set_content(response.dump(), "application/json");
    });

    // Handle stats endpoint
    svr.Get("/handles/stats", [](const httplib::Request&, httplib::Response& res) {
        json response = {
            {"success", true},
            {"handles", {
                {"art", HandleManager::art.Size()},
