...
export interface CreateRectangleRequest {
  /** X coordinate of the rectangle origin in points */
  x: number;
  /** Y coordinate of the rectangle origin in points */
  y: number;
...
export interface CreateRectangleResponse {
  /** Art handle ID registered with HandleManager::art */
  handle: number;
  /** UUID assigned to the new art object */
  uuid: string;
}
...
export interface CreateEllipseRequest {
  /** Center X coordinate in points */
  cx: number;
  /** Center Y coordinate in points */
  cy: number;
...
export interface CreateEllipseResponse {
  /** Art handle ID registered with HandleManager::art */
  handle: number;
  /** UUID assigned to the new art object */
  uuid: string;
...
export interface CreateLineRequest {
  /** Start point X coordinate in points */
  x1: number;
  /** Start point Y coordinate in points */
  y1: number;
  /** End point X coordinate in points */
  x2: number;
  /** End point Y coordinate in points */
  y2: number;
}
...
export interface ShowFileSaveDialogRequest {
  /** Default file name to populate in the dialog */
  defaultName?: string;
  /** Dialog window title */
  title?: string;
  /** Allowed file type extensions (e.g. ["ai", "pdf", "svg"]) */
  fileTypes?: string[];
}

export interface ShowFileSaveDialogResponse {
...
export interface ExportSvgViaActionRequest {
  /** Destination file path for SVG output. If empty, uses a temp file. */
  outputPath?: string;
}

export interface ExportSvgViaActionResponse {
  /** Whether export succeeded */
  success: boolean;
  /** SVG file content as string (only if outputPath was empty/omitted) */
  svgContent: string;
  /** Path where SVG was written */
  filePath: string;
...
export interface ExportSelectionAsSvgResponse {
  /** Whether export succeeded */
  success: boolean;
  ...
  /** Path where SVG was written */
  filePath: string;
  /** Bounding box of exported art: { left, top, right, bottom, width, height } */
  artBounds: Record<string, unknown>;
  /** Error or status message */
  message: string;
}
...
export interface ExportSvgViaFileFormatRequest {
  /** Destination file path for SVG output. If empty, uses a temp file. */
  outputPath?: string;
}

export interface ExportSvgViaFileFormatResponse {
  /** Whether export succeeded */
  success: boolean;
  /** SVG file content as string (only if outputPath was empty/omitted) */
  svgContent: string;
...
/** POST /api/art/rectangle - Create a rectangle path art object at the given position and size. Uses AIPathSuite to create a new path and set four corner segments. */
export async function CreateRectangle(params: CreateRectangleRequest): Promise<CreateRectangleResponse> {
  return fetchRoute<CreateRectangleResponse>('POST', getApiUrl('/api/art/rectangle'), JSON.stringify(params));
}

/** POST /api/art/ellipse - Create an ellipse path art object with the given center and radii. Uses AIPathSuite to create a new path with bezier-approximated ellipse segments. */
export async function CreateEllipse(params: CreateEllipseRequest): Promise<CreateEllipseResponse> {
  return fetchRoute<CreateEllipseResponse>('POST', getApiUrl('/api/art/ellipse'), JSON.stringify(params));
}

/** POST /api/art/path - Create a new path art object from an array of segments. Uses AIPathSuite to create a new path and set segments. Segment format matches GetPathSegments/SetPathSegments. */
export async function CreatePath(params: CreatePathRequest): Promise<CreatePathResponse> {
  return fetchRoute<CreatePathResponse>('POST', getApiUrl('/api/art/path'), JSON.stringify(params));
}

/** POST /api/art/line - Create a line (open path with two segments) between two points. Uses AIPathSuite to create a new path with start and end segments. */
export async function CreateLine(params: CreateLineRequest): Promise<CreateLineResponse> {
  return fetchRoute<CreateLineResponse>('POST', getApiUrl('/api/art/line'), JSON.stringify(params));
}

/** POST /api/dialog/save-file - Show a native file save dialog. Uses AIUserSuite::PutFileDialog to display a platform save dialog and return the selected path. */
export async function ShowFileSaveDialog(params: ShowFileSaveDialogRequest): Promise<ShowFileSaveDialogResponse> {
  return fetchRoute<ShowFileSaveDialogResponse>('POST', getApiUrl('/api/dialog/save-file'), JSON.stringify(params));
}

/** POST /api/export/svg-via-action - Export current document as SVG using AIActionManagerSuite::PlayActionEvent with adobe_exportDocument action and kDialogOff. Returns SVG file content as a string. */
export async function ExportSvgViaAction(params: ExportSvgViaActionRequest): Promise<ExportSvgViaActionResponse> {
  return fetchRoute<ExportSvgViaActionResponse>('POST', getApiUrl('/api/export/svg-via-action'), JSON.stringify(params));
}

/** POST /api/export/selection-svg - Export only the current selection as SVG. Hides all layers, duplicates selection to a temp layer, exports, reads back SVG content, then restores layer state. */
export async function ExportSelectionAsSvg(): Promise<ExportSelectionAsSvgResponse> {
  return fetchRoute<ExportSelectionAsSvgResponse>('POST', getApiUrl('/api/export/selection-svg'));
}

/** POST /api/export/svg-via-fileformat - Export current document as SVG using AIFileFormatSuite::GoExport with SVG format parameters. Returns SVG file content as a string. */
export async function ExportSvgViaFileFormat(params: ExportSvgViaFileFormatRequest): Promise<ExportSvgViaFileFormatResponse> {
  return fetchRoute<ExportSvgViaFileFormatResponse>('POST', getApiUrl('/api/export/svg-via-fileformat'), JSON.stringify(params));
}
