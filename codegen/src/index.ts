...
const BLOCKED_FUNCTIONS: Set<string> = new Set([
    ...
    // AINotifierSuite - void* params, const char** outputs, missing types
    'GetNotifierName', // const char** output parameter misclassified
    'GetNotifierType', // const char** output parameter misclassified
    'Notify',
    'AddNotifierEx',   // AINotifierOptions type doesn't exist in SDK headers
    'GetNotifierOptions', // AINotifierOptions type doesn't exist in SDK headers
...
`;
    ...
    // Generate _bridge.ts shim (delegates callCpp to Bridge instance)
    const bridgeShim = `/**
...
`;
    await fs.writeFile(path.join(tsOutputDir, '_bridge.ts'), bridgeShim, 'utf-8');
    logger.debug('Generated TypeScript _bridge.ts shim');

    // Generate _config.ts shim (provides getApiUrl and sdkConfig for route/event files)
    const configShim = `/**
...
`;
    await fs.writeFile(path.join(tsOutputDir, '_config.ts'), configShim, 'utf-8');
    logger.debug('Generated TypeScript _config.ts shim');
}
