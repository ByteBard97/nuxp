...
import { BridgeError, getBridgeInstance, SSEAdapter, buildServerUrl } from '@nuxp/sdk';
...
let sseClient: SSEAdapter | null = null;

// Check if running in Tauri and start SSE connection
onMounted(() => {
  isTauri.value = !!(window as unknown as { __TAURI__?: unknown }).__TAURI__;

  // Create and connect SSE adapter
  sseClient = new SSEAdapter({ serverUrl: buildServerUrl(NUXP_PORT) });
  sseClient.connect();
  eventLoopActive.value = true;

  eventUnsubscribe = sseClient.onAll((event: { type: string; data?: unknown }) => {
...
async function createTestRectangle(): Promise<void> {
  ...
  try {
    ...
    const newArtResult = await getBridgeInstance().callSuite<{ newArt: number }>(
      'AIArtSuite',
      'NewArt',
      {
        type: kPathArt,
    ...
    await getBridgeInstance().callSuite('AIArtSuite', 'SetArtName', {
      art: artHandle,
      name: 'Hello NUXP',
    });
    ...
    await getBridgeInstance().callSuite('AIPathSuite', 'SetPathSegments', {
      path: artHandle,
      segNumber: 0,
      count: 4,
      segments: segments,
    ...
    await getBridgeInstance().callSuite('AIPathSuite', 'SetPathClosed', {
      path: artHandle,
      closed: true,
    });
